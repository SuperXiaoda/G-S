...
public class AHBottomNavigation extends FrameLayout {
    ...
    private
    @ColorInt
    int titleColorActive, titleColorInactive;
    private
    @ColorInt
    int coloredTitleColorActive, coloredTitleColorInactive;
    ...
    private void updateNotifications(boolean updateStyle, int itemPosition) {
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            View dot = views.get(i).findViewById(R.id.bottom_navigation_dot);
            ...
            if (updateStyle) {
                ...
                if (notificationBackgroundDrawable != null) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                        ...
                        dot.setBackgroundDrawable(notificationBackgroundDrawable);
                    }
                    ...
                    Drawable defautlDotDrawable = ContextCompat.getDrawable(context, R.drawable.notification_dot_background);
                    Drawable newDrawable = AHHelper.getTintDrawable(defautlDrawable, notificationBackgroundColor, forceTint);
                    Drawable newDotDrawable = AHHelper.getTintDrawable(defautlDotDrawable, notificationBackgroundColor, forceTint);
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                        notification.setBackground(newDrawable);
                        dot.setBackground(newDotDrawable);
                    } else {
                        notification.setBackgroundDrawable(newDrawable);
                        dot.setBackgroundDrawable(newDotDrawable);
                    }
                }
            }
...
//            }
            ...
            String oldNotification = notification.getText().toString();
            if (notifications[i].length() == 0 && notification.getText().length() > 0) {// 隐藏
                notification.setText("");
                if (oldNotification.length() > 0) {
                    if (DOT.equals(oldNotification)) {
                        // 隐藏点
                        dot.animate()
                                .scaleX(0)
                                .scaleY(0)
            ...
            } else if (notifications[i].length() > 0) { // 显示
                if (DOT.equals(notifications[i])) { // 显示点
                    notification.setScaleX(0);
                    notification.setScaleY(0);
