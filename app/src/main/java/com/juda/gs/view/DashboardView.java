...
public class DashboardView extends View {
    ...
    // 单位
    private String mUnit = "km/h";
    private String mRealTimeStr = String.valueOf(mMin);// 实时读数文字
    // 表头
    private String mHeaderText = "";
    // 实时读数
    private float mRealTimeValue = mMin;
    // 是否显示实时读数
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int max = (int) Math.max(
                height1,
                Math.max(mSweepAngle + mRadius, mSweepAngle + mRadius)
        );
    ...
    protected void onDraw(Canvas canvas) {
         ...
         */
        mPaint1.setStrokeWidth(dp2px(1));
        ...
        canvas.save();
        canvas.drawLine(x0, y0, x1, y1, mPaint1);
        ...
        canvas.drawLine(x0, y0, x2, y2, mPaint1);
        ...
        for (int i = 0; i < mTexts.length; i++) {
            mPaint1.getTextBounds(mTexts[i], 0, mTexts[i].length(), mRectText);
            // 粗略把文字的宽度视为圆心角2*θ对应的弧长，利用弧长公式得到θ，下面用于修正角度
            float θ2 = (float) (180 * mRectText.width() / 2 /
                    (Math.PI * (mRadius - mLength2 - mRectText.height())));

            mPath.reset();
            mPath.addArc(
                    mRectFInnerArc,
                    mStartAngle + i * (mSweepAngle / mSection) - θ2, // 正起始角度减去θ使文字居中对准长刻度
                    mSweepAngle
            );

            canvas.drawTextOnPath(mTexts[i], mPath, 0, 0, mPaint1);
        }
        ...
        canvas.drawCircle(mCenterX, mCenterY, dp2px(2), mPaint1);

        /**
         * 画实时度数值
         */
        if (isShowValue) {
            mPaint1.setTextSize(sp2px(16));
            mPaint1.setTextAlign(Paint.Align.CENTER);
            mPaint1.setColor(ContextCompat.getColor(getContext(), R.color.color_green));
            String value = mRealTimeValue + mUnit;
            mPaint1.getTextBounds(value, 0, value.length(), mRectText);
            canvas.drawText(value, mCenterX, mCenterY + mPSRadius + mRectText.height() * 2, mPaint1);
        }
    }
     ...
     * 设置实时数据
     */
    public void setRealTimeValueNow(float realTimeValue, String realTimeStr) {
        if ((mRealTimeValue == realTimeValue || realTimeValue < mMin || realTimeValue > mMax) && mRealTimeStr.equals(realTimeStr)) {
            return;
        } else {
            mRealTimeValue = realTimeValue;
            mRealTimeStr = realTimeStr;
            postInvalidate();
        }
     ...
     * 设置实时数据
     */
    public void setRealTimeValue(float realTimeValue, String realTimeStr) {
        mRealTimeValue = realTimeValue;
        mRealTimeStr = realTimeStr;
    }
     ...
     * 设置最小值
     */
    public void serMinNumber(int min) {
        this.mMin = min;
    }
